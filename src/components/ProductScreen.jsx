                ...
                produto ? <div className='flex justify-between h-full w-full'>
                    <div className=' py-20 px-10 w-full h-full flex  gap-[20px] relative'> 
                        ...
                        <div className='flex flex-col gap-9'>
                            ...
                            <p className='text-[30px] max-h-[450px] overflow-y-auto esconderScroll'>{produto.description}</p>
                            <p className='font-black text-[40px] absolute bottom-20 left-10'>R${produto.price.toFixed(2)}</p>
                            <button onClick={handleActionBackHome } className='py-4 px-[70px] w-fit bg-blue-500 text-white rounded-[10px] font-semibold hover:scale-105 transition-all hover:bg-green-500 shadow-md absolute bottom-[90px] right-10 '>
                                Ver Outros
                            </button>
                        </div>
                    ...
                    <div className='w-4/6 h-full shadow-md p-20 flex flex-col gap-7 relative'>
                        ...
                        <input onChange={changeValueCep} placeholder='CEP' className='outline-none bg-transparent text-black py-2 px-3 border border-black/20 rounded-[5px] w-[250px] ' />
                            ...
                            <button onClick={handleFinish} className='py-4 px-[70px] w-fit bg-blue-500 text-white rounded-[10px] font-semibold hover:scale-105 transition-all hover:bg-green-500 shadow-md absolute bottom-[90px] right-10 '>
                                Comprar
                            </button>
                    </div>
                </div> :null
